...
static int lua_basic_print(lua_State* L) {
    ...
    for (int i = 1; i <= n; i++) {
        // Push the argument to io.write
        lua_pushvalue(L, i);
        // Call io.write with 1 argument
        lua_call(L, 1, 0);
    }
...
void registerTerminalBindings(lua_State* L) {
    ...
    if (lua_isnil(L, -1)) {
        ...
        printf("DEBUG: FasterBASICT registering basic_input_at (FBRunner3 version not found)\n");
        lua_pushcfunction(L, [](lua_State* L) -> int {
            printf("DEBUG: FasterBASICT basic_input_at called\n");
            int x = luaL_checkinteger(L, 1);
            int y = luaL_checkinteger(L, 2);
            std::string prompt = "";
            ...
            printf("DEBUG: FasterBASICT INPUT_AT x=%d, y=%d, prompt='%s'\n", x, y, prompt.c_str());
            ...
            // Read input
            std::string result = terminal_read_line("");
            printf("DEBUG: FasterBASICT returning: '%s'\n", result.c_str());
            lua_pushstring(L, result.c_str());
            return 1;
        });
        lua_setglobal(L, "basic_input_at");
    } else {
        printf("DEBUG: FasterBASICT found existing basic_input_at, not overriding\n");
