...
public:
    ...
    PrintStatement() : trailingNewline(true), hasUsing(false) {}
...
public:
    ...
    FunctionStatement(const std::string& name, TokenType suffix = TokenType::UNKNOWN)
        : functionName(name), returnTypeSuffix(suffix) {}

    void addParameter(const std::string& param, TokenType type = TokenType::UNKNOWN) {
        parameters.push_back(param);
