...
StatementPtr Parser::parseCaseStatement() {
    ...
    // Expect ENDCASE
    consume(TokenType::ENDCASE, "Expected ENDCASE to close CASE statement");
...
StatementPtr Parser::parseDimStatement() {
    ...
    do {
        ...
        if (!stmt->arrays.empty()) {
            stmt->arrays.back().typeSuffix = mergeTypes(suffix, asType, varName);
        }
...
StatementPtr Parser::parseLocalStatement() {
    ...
    do {
        ...
        if (!stmt->variables.empty()) {
            stmt->variables.back().typeSuffix = mergeTypes(suffix, asType, varName);
        }
...
StatementPtr Parser::parseFunctionStatement() {
    ...
    if (current().type != TokenType::RPAREN) {
        do {
            ...
            stmt->addParameter(paramName, paramType);
        } while (match(TokenType::COMMA));
    }
...
StatementPtr Parser::parsePrintAtStatement() {
    ...
    // Check for PRINT_AT USING syntax
    if (match(TokenType::USING)) {
        stmt->hasUsing = true;
        stmt->formatExpr = parseExpression();
